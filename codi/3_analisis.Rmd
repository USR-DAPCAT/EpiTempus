...
dt_plana<-dt_plana %>% mutate(year_inclusio=stringr::str_sub(dtindex,1,4)) %>% mutate(year_inclusio=if_else(year_inclusio<=2007,"<=2007",year_inclusio))
...
descrTable(formu,data=dt_plana_exc,show.p.overall = F,hide = "No",max.ylev = 20,max.xlev = 20) %>% 
...
dt_per_fx<-function(primer_fx="First.event_Metformina") {
  dt_plana_exc %>% filter(!!sym(primer_fx)=="Yes") %>% 
...
dt_temp2<-dt_plana_exc %>% summarise_at(farmcacs1r,~sum(.=="Yes")) %>% 
...
dt_temp2 %>% left_join(dt_temp1,by="First drug") %>% etiquetar_taula(camp = "First drug",taulavariables = conductor) %>% 
