...
dt_index<-
  ...
  filter(entrada>=20060101 & sortida<=20181231)  %>%  # Filtro per usuari actiu en data index
  select(-c(entrada,sortida,kk))
...
left_join(dt_poblacio,by="idp")%>%
 left_join(dtagr_INCLUSIONS_2018_EPIPEU_AP_HOSP,by=c('idp'))%>%
  left_join(dtagr_problemes_2018_EPIPEU_AP_HOSP1a,by=c('idp'))%>%
   left_join(dtagr_problemes_2018_EPIPEU_AP_HOSP1b,by=c('idp'))%>%
    left_join(dtagr_problemes_2018_EPIPEU_AP_HOSP1c,by=c('idp'))%>%
     left_join(dtagr_problemes_2018_EPIPEU_AP_HOSP1d,by=c('idp'))%>%
      left_join(dtagr_problemes_2018_EPIPEU_AP_HOSP2,by=c('idp'))%>%
       left_join(dtagr_problemes_2018_EPIPEU_AP_HOSP2b,by=c('idp'))%>%
        left_join(dtagr_problemes_2018_EPIPEU_AP_HOSP3,by=c('idp'))%>% 
         left_join(dtagr_problemes_2018_EPIPEU_AP_HOSP4,by=c('idp'))
